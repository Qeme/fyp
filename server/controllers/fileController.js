...
export const getAllFiles = async (req, res) => {
  ...
  try {
    ...
    if (!files || files.length === 0) {
      return res
        .status(404)
        .json({ error: topic ? `No ${topic} files exist` : "No files exist" });
    }
...
export const getAnImage = async (req, res) => {
  ...
  try {
    ...
    if (
      file[0].metadata.type === "image/jpeg" ||
      file[0].metadata.type === "image/png"
    ) {
...
export const uploadFile = (req, res) => {
  ...
  uploadStream
    .on("finish", async function () {
      ...
      // Prepare file details for JSON response
      const fileDetails = {
      ...
      if (topic === "receipt") {
        try {
          ...
          res.status(200).json({ payment, file: fileDetails });
        } catch (error) {
          console.error("Error creating payment:", error);
          res
            .status(500)
            .json({ error: "An error occurred while creating payment" });
        }
      } else {
        res
          .status(200)
...
//   const files = req.files.files; // `files` is an array of files
//   const { _id } = req.params;
//   const user = req.user;
//   const { topics } = req.fields; // topics should be an array matching the files
...
export const deleteFile = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Successfully deleted the file", file: _id });
